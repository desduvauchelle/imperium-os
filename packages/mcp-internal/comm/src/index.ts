...
export interface CommAdapter {
	readonly channel: CommChannel
	send(recipient: string, message: string): Promise<Result<{ messageId: string }>>
...
export class DiscordAdapter implements CommAdapter {
	...
	constructor(webhookUrl: string, fetchFn: FetchFn = globalThis.fetch.bind(globalThis)) {
		this.webhookUrl = webhookUrl
		this.fetchFn = fetchFn
	}

	async send(_recipient: string, message: string): Promise<Result<{ messageId: string }>> {
		try {
...
export class TelegramAdapter implements CommAdapter {
	...
	constructor(botToken: string, defaultChatId?: string, fetchFn: FetchFn = globalThis.fetch.bind(globalThis)) {
		this.botToken = botToken
		this.defaultChatId = defaultChatId
		this.fetchFn = fetchFn
	}

	async send(recipient: string, message: string): Promise<Result<{ messageId: string }>> {
		const chatId = recipient || this.defaultChatId
		if (!chatId) return { ok: false, error: new Error('No chat_id specified and no default configured') }
		try {
	...
	async read(limit: number): Promise<Result<readonly CommMessage[]>> {
		try {
			...
			const res = await this.fetchFn(url)
			if (!res.ok) return ok([])
			const data = (await res.json()) as { result?: Array<{ message?: { message_id: number; from?: { username?: string }; text?: string; date?: number } }> }
...
export class WhatsAppAdapter implements CommAdapter {
	...
	constructor(accountSid: string, authToken: string, fromNumber: string, fetchFn: FetchFn = globalThis.fetch.bind(globalThis)) {
...
export class EmailAdapter implements CommAdapter {
	...
	async send(recipient: string, message: string): Promise<Result<{ messageId: string }>> {
		return this.sendEmail({ to: recipient, subject: 'Imperium OS Notification', body: message })
	}
		...
		try {
			switch (call.toolName) {
				case 'send_message': {
					...
					if (!channel) return this.toolError(call.requestId, start, 'Missing required argument: channel')
					const recipient = args['recipient'] as string | undefined
					if (!recipient) return this.toolError(call.requestId, start, 'Missing required argument: recipient')
					const message = args['message'] as string | undefined
					if (!message) return this.toolError(call.requestId, start, 'Missing required argument: message')

					const adapter = this.adapters.get(channel)
					if (!adapter) return this.toolError(call.requestId, start, `No adapter registered for channel: ${channel}`)
				...
				case 'list_channels': {
					return ok(this.toolResult(call.requestId, start, this.getAvailableChannels()))
				}
				default:
