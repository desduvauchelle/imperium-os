...
import type { ChatMessageRepository, MemoryBlockRepository } from '@imperium/core-db'
...
export class MemorySnapshotManager {
  ...
  async createSnapshot(
    ...
    const blockId = createMemoryBlockId(`mb-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`)
