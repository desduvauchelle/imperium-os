...
import type { McpServer, McpToolCall, McpToolResult } from '@imperium/shared-types'
import { ok, err, isOk, isErr } from '@imperium/shared-types'
...
function makeServer(overrides: Partial<McpServer> = {}): McpServer {
	return {
		...
		version: '1.0.0',
		tools: [
			{ name: 'tool_a', description: 'Tool A', inputSchema: {}, requiresPermission: 'file-read' },
			{ name: 'tool_b', description: 'Tool B', inputSchema: {}, requiresPermission: 'file-write' },
		],
		enabled: true,
		...overrides,
	}
...
describe('McpRegistry', () => {
	...
	test('dispatch routes to correct server', async () => {
		const reg = new McpRegistry()
		const dispatchFn = async (call: McpToolCall) => ok(makeResult(call.requestId))
	...
	test('dispatch returns error for disabled server', async () => {
		const reg = new McpRegistry()
		reg.register(makeServer({ enabled: false }), async () => ok(makeResult('r')))
		const result = await reg.dispatch(makeCall())
		expect(isErr(result)).toBe(true)
		if (!result.ok) {
	...
	test('dispatch routes to correct server among multiple', async () => {
		const reg = new McpRegistry()
		reg.register(
			makeServer({ id: 'server-a', tools: [{ name: 'ta', description: '', inputSchema: {}, requiresPermission: 'file-read' }] }),
			async () => ok({ ...makeResult('r'), data: 'from-a' }),
		)
		reg.register(
			makeServer({ id: 'server-b', tools: [{ name: 'tb', description: '', inputSchema: {}, requiresPermission: 'file-write' }] }),
