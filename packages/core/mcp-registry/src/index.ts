import type { McpServer, McpToolCall, McpToolResult, McpServerSummary, Result } from '@imperium/shared-types'
...
export type McpDispatchFn = (call: McpToolCall) => Promise<Result<McpToolResult>>
...
export class McpRegistry {
	...
	async dispatch(call: McpToolCall): Promise<Result<McpToolResult>> {
		...
		if (!entry) {
			return err(new Error(`No MCP server registered with id '${call.serverId}'`))
		}
